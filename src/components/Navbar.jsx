  ...
  const MenuList = ({ isMobile }) => (
    ...
    >
      {navLinks.map(({ id, title }) => (
        ...
        >{
            id === "resume" ? <a href="/Pooja_Jadhav_CSE_2024_FullStack Java  Developer.pdf" target="_blank">{title}</a>
              :
              <a href={`#${id}`}>{title}</a>
          }</li>
      ))}
      ...
      <div className="w-full flex items-center justify-between max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="cursor-pointer flex flex-col">
            <span className="font-semibold">Pooja Jadhav</span>
            <span className="text-xs italic text-secondary">
              jpoojas352@gmail.com
            </span>
          </p>
        </Link>
